 ...
 */
export async function diffComponents(oldComponents, newComponents, options = {}) {
    // Initialize stats
    const stats = {
        added: 0,
        removed: 0,
    ...
    for (const family of families) {
        ...
        for (const oldHook of oldHooks) {
            const matchIndex = newHooks.findIndex(h => h.className === oldHook.className &&
        ...
        for (const newHook of newHooks) {
            const exists = oldHooks.some(h => h.className === newHook.className &&
    ...
    if (options.saveToFile && options.outputPath) {
        try {
            ...
            await fs.writeFile(options.outputPath.replace(/\.txt$/, '') + '.json', JSON.stringify(results, null, 2));
        }
        catch (error) {
 ...
 * @returns Stats about the diff
 */
export async function diffComponentFiles(oldPath, newPath, options = {}) {
    try {
        // Read files
        const oldContent = await fs.readFile(oldPath, 'utf-8');
        const newContent = await fs.readFile(newPath, 'utf-8');
        // Parse JSON
        const oldComponents = JSON.parse(oldContent);
        const newComponents = JSON.parse(newContent);
        // Generate diff
