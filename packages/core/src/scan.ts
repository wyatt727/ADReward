...
import { ComponentMap as KtIRComponentMap, KtirScanner } from './ktir.js';
import fs from 'fs/promises';
import { execSync } from 'child_process';
import { workerPool } from './utils/workerPool.js';
import { timing } from './utils/timing.js';
import { fileURLToPath } from 'url';
import { pipelineHooks, ScanContext } from './hooks.js';
 ...
 */
export async function hasKotlinMetadata(dexDir: string): Promise<boolean> {
  try {
...
export async function scan(
  ...
  return await timing.time('scan-total', async () => {
    ...
    if (options.useKtirScanner === undefined) {
      options.useKtirScanner = await timing.time('kotlin-metadata-check', 
        () => hasKotlinMetadata(dexDir)
      );
    ...
    let ktirPromise: Promise<KtIRComponentMap> | null = null;
    ...
    if (!options.useKtirOnly) {
      await timing.time('smali-scan', async () => {
        ...
        // Get all smali files
        const files = await timing.time('smali-file-discovery', 
          () => fg(['**/*.smali'], { cwd: dexDir, absolute: true })
        );
        ...
        await timing.time('smali-worker-processing', async () => {
          const tasks = chunks.map(chunk => 
            workerPool.run<{ files: string[], families: string[] }, WorkerResult>({
              files: chunk, 
              families 
            })
          );
    ...
    if (ktirPromise) {
      try {
        const ktirResults = await timing.time('kotlin-ir-scan', 
          () => ktirPromise!
        );
        ...
        await timing.time('results-merge', async () => {
          for (const [family, hooks] of Object.entries(ktirResults)) {
            ...
            for (const hook of hooks) {
              ...
              const existingIndex = results[family].findIndex(
                h => h.className === hook.className && h.methodName === hook.methodName
              );
