...
import { pipelineHooks, DecompileContext } from './hooks.js';
 ...
 */
export async function decompile(apk: string, { out, generateJar = false, forceDecompile = false }: DecompileOptions): Promise<string> {
  // Create decompile context for hooks
  const decompileContext: DecompileContext = {
    apkPath: apk,
    outDir: out,
 ...
 */
async function generateDecompileCache(dexDir: string, apkHash: string): Promise<void> {
  ...
  try {
    ...
    fs.writeFileSync(
      path.join(dexDir, 'dex.hash.json'), 
 ...
 * @param dexDir Directory containing the decompiled DEX files
 */
async function generateClassesJar(apk: string, dexDir: string): Promise<void> {
